...
public class PurchaseAnalysis {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		props.put(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, "192.168.16.172:9092");
		props.put(StreamsConfig.ZOOKEEPER_CONNECT_CONFIG, "192.168.16.172:2181");
...
//				.foreach((String itemName, OrderUserItem orderUserItem) -> System.out.printf("%s-%s-%s-%s\n", itemName, orderUserItem.itemAddress, orderUserItem.userName, orderUserItem.userAddress))
		...
		KTable<String, String> kTable = orderStream
				...
				//.filter((String item, OrderUserItem orderUserItem) -> StringUtils.compare(orderUserItem.userAddress, orderUserItem.itemAddress) == 0)
//				.foreach((String itemName, OrderUserItem orderUserItem) -> System.out.printf("%s-%s-%s-%s\n", itemName, orderUserItem.itemAddress, orderUserItem.userName, orderUserItem.userAddress))
			.map((String item, OrderUserItem orderUserItem) -> KeyValue.<String, String>pair(orderUserItem.userAddress +","+ orderUserItem.gender,itemGenOutStr(orderUserItem)))// (Double)(orderUserItem.quantity * orderUserItem.itemPrice)))
		...
		System.out.printf("地区,性别,定单总数,商品总数,总金额\n");
	...
	private static String itemGenOutStr(OrderUserItem oui) {
		int ordN = 1;
		int spn = oui.quantity;
		Double sumD =(Double)(oui.quantity * oui.itemPrice); 
